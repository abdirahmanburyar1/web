...
const BASE = "/platform";
const SIDEBAR_COLLAPSED_KEY = "aquatrack-platform-sidebar-collapsed";

const nav = [
  { href: `${BASE}/dashboard`, label: "Dashboard", icon: IconDashboard, section: "Overview" },
  { href: `${BASE}/tenants`, label: "Tenants", icon: IconBuilding, section: "Tenants" },
  { href: `${BASE}/reports`, label: "Reports & revenue", icon: IconReports, section: "Reports" },
];
...
export function PlatformSidebar({
  ...
  const isActive = (href: string) =>
    pathname === href || (href === `${BASE}/tenants` && pathname.startsWith(`${BASE}/tenants/`));

  const linkClass = (active: boolean) =>
    `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
      active
        ? "bg-cyan-500 text-white shadow-sm dark:bg-cyan-600 dark:text-white"
      ...
      >
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-slate-200/80 px-3 dark:border-slate-700/80">
          <div className="relative shrink-0">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-cyan-500 text-sm font-semibold text-white shadow-md dark:bg-cyan-600">
              A
            </div>
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-xs text-slate-500 dark:text-slate-400">Platform</p>
            </div>
          )}
          {collapsed ? (
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyan-500 text-white transition hover:bg-cyan-600 dark:bg-cyan-600 dark:hover:bg-cyan-500"
        ...
        <div className="border-t border-slate-200/80 p-3 dark:border-slate-700/80">
          ...
          {collapsed ? (
            <div className="flex flex-col gap-1">
              <Link href="/" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700/60" title="Home">
              ...
              <Link
                ...
                className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700/60"
                title="Sign out"
              >
            ...
            <>
              <Link href="/" onClick={onClose} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-700/60 dark:hover:text-slate-200">
                <IconHome className="size-5" /> Home
              </Link>
              <Link
                href={`${BASE}/login`}
                onClick={() => {
